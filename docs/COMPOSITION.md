...
- **Rules** — `precondition token(s) -> result token`, each annotated with a
  calibrated exploit-reliability weight `p ∈ (0,1]`:
   ...
   These are the **measured** weights (Laplace-MLE of observed exploit
   outcomes on the local slice of `evaluation/corpus.local.json`; see
   the calibration comment in `src/compose.c`). Rules R8 and R10 have no
...
(`evaluation/calibrate.py`, `α=1`). On the local corpus slice the calibrated weights
lower the Brier score from the seed value (see §6.2 of the evaluation).
...
- `src/compose.c` — token enum, `RULES[]`, `map_link()`, `fixpoint_from()`,
  `zp_compose_json()`.
...
- Calibrated `p` values are fitted on the local slice of
  `evaluation/corpus.local.json`; per-rule
